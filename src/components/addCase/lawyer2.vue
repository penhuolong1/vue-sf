<template>
  ...
  >
      ...
      <el-row>
        <el-col :span="20">
          <el-form-item label="证件号码" prop="agentIdentiCard">
            <!-- <el-input v-model="form.agentIdentiCard" type="text" @input="getIdCard" /> -->
            <encryptInfo :id="form.lawyerId" ref="encryptInfo1" v-model="form.agentIdentiCard" type="agentIdentiCard" :litigant-type="2" :placeholder="'请输入证件号码'" @input="getIdCard" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
      ...
      <el-row>
        <el-col :span="20">
          <el-form-item label="手机号码" prop="agentMobile">
            ...
            <encryptInfo :id="form.lawyerId" ref="encryptInfo2" v-model="form.agentMobile" type="agentMobile" :litigant-type="2" :placeholder="'请输入手机号码'" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button v-if="!isChange" type="primary" :loading="isLoading" @click="save">确 定</el-button>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        otherAuthority: [
          { required: true, message: '请输入其他委托事项', trigger: 'blur' }
        ],
        agentEmail: [
  ...
  methods: {
    ...
    save() {
      this.$refs['form'].validate(valid => {
        ...
        this.form.authority = this.checkList.join(',')
        if (this.fileList && this.fileList.length > 0) {
          this.form.frontImage = this.fileList[0].url
          this.form.backImage = this.fileList[1] ? this.fileList[1].url : ''
        }
        this.form.litigantId = this.lawParentId
        this.form.years = Number(this.form.years)
