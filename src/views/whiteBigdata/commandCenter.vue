<template>
  <el-row style="position: relative;">
    <a href="#" style="position:absolute;top:-60px;left:30px;color:#fff;cursor:pointer;z-index:10" @click="goBack()"><Icon type="ios-undo-outline" size="24" />返回</a>
    ...
    <el-col :span="8" class="center-wrapper">
      <div ref="picBox" class="center-wrapper-top" :style="{height:getHeight + 'px'}">
        <div class="content" :style="{paddingTop:getpadding +16+ 'px'}">
          <p class="text">法院新收案件数</p>
          <h1 class="num" style="padding-top:15px;padding-bottom:15px;">7.8万件</h1>
...
import { getMediater, login } from '../../api/common/common.js'
import md5 from 'md5'
...
export default {
  ...
  methods: {
    ...
    goBack() {
      const params = {
        username: '厦门市诉非联动中心',
        password: md5('sfld2020')
      }
      login(params).then(res => {
        if (res.state == 100) {
          localStorage.setItem(`${SYS_NAME}-userRole`, 1)
          this.$store.commit('SET_USERROLE', 1)
          this.$router.push({ path: '/otherHome/index' })
        }
      })
    }
  }
...
.xm-bigdata-wrapper{
  ...
  .center-wrapper {
    ...
    .center-wrapper-bottom {
      ...
      background-size: 100% 100%;
      background-repeat: no-repeat;
      background-image: url('../../assets/bigData/xm/8备份备份.png');
      // position: absolute;
      .one{
        height: 50%;
        .contentText{
