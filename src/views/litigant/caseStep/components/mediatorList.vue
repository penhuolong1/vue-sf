    ...
    <ul v-show="!isDetail" v-loading="loading">
      <li v-for="(item, index) in courtData" :key="index" class="select-item" @click="selectCourt(item)">
        ...
        <div class="select-item-right">
          ...
          <div class="num">解纷数量：{{ item.caseNum }}</div>
    ...
    <div v-show="isDetail" class="detail-wrapper">
      <div class="wrapper">
        ...
        <div class="wrapper-right">
          ...
          <div class="num">解纷数量：{{ detailObj.caseNum }}</div>
        </div>
      </div>
      <div class="content">
        <p>简介:{{ detailObj.introduction }}</p>
      </div>
    </div>
  </div>
</template>
...
<script>
import { getTopThreeMediator } from '@/api/litigant/litigant'
export default {
  components: {
  ...
  computed: {
    courtData1() {
      return this.$store.state.litigant.courtData.id
    }
  },
  watch: {
    courtData1() {
  ...
  methods: {
    ...
    getTopThreeCourt() {
      ...
      if (this.$store.state.litigant.courtData.id) {
        params.courtId = this.$store.state.litigant.courtData.id
        params.caseId = ''
      } else {
        params.caseId = localStorage.getItem('lawCaseId')
        params.courtId = ''
      }
      this.loading = true
      getTopThreeMediator(params).then(res => {
        this.loading = false
        if (res.state === 100) {
